...
class LinposSIDRun:
    def __init__(self, amp = VEL_AMPLITUDE, ramp_t = 5.0, fall_t = 2.0, cycles = 1):
        self.amp = amp
        self.ramp_t = ramp_t
        self.fall_t = fall_t
        self.cycles = cycles
...
# Input object list
sysid_inputs = [
    LinposSIDRun(VEL_AMPLITUDE, 10.0, 10.0, 100.0)
]
...
class SIFurataLinear(Node):
    def __init__(self):
        ...
        self._odrv.axis0.controller.config.control_mode = ControlMode.POSITION_CONTROL
    ...
    def cyclic_control(self):
        ...
        if self.is_rising:
            # Set position based on linear ramp
            pos_cmd =(cur_sysidrun.amp)*phase_t
            self._odrv.axis0.controller.input_pos = -pos_cmd
            ...
            if phase_t >= cur_sysidrun.ramp_t:
                ...
                self.peak_pos = pos_cmd
        else:

            # Check for cycle end
            pos_cmd = self.peak_pos - (cur_sysidrun.amp)*phase_t
        ...
        (self.tel_dict["tau_cmd"]).append(pos_cmd)
        ...
        if self.cycle_counter >= (cur_sysidrun.cycles):
            ...
            self._odrv.axis0.controller.input_pos = 0.0
    ...
    def save_plot_log(self, save_dir='sys_id_data'):
        ...
        data_filename = os.path.join(save_dir, f'linpos_sys_id_data_{timestamp}.pkl')
        ...
        data_filename = os.path.join(save_dir, f'linpos_sys_id_BYFREQ_{timestamp}.pkl')
        with open(data_filename, 'wb') as f:
        ...
        # Save figure
        plot_filename = os.path.join(save_dir, f'linpos_si_motor_plot_{timestamp}.png')
        ...
        # Save figure
        plot_filename = os.path.join(save_dir, f'linpos_si_motorPhase_plot_{timestamp}.png')
        plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
        print(f"Plot saved to: {plot_filename}")
        ...
        # Save figure
        plot_filename = os.path.join(save_dir, f'linpos_si_Pend_plot_{timestamp}.png')
        plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
        print(f"Plot saved to: {plot_filename}")
...
def main():
    ...
    try:
        ...
        DATA_DIR = Path.home() / "ws_ros2_odrive" / "src" / "ps5_odrive_control" / "ps5_odrive_control" / "sys_id" / "linpos_si_data"
